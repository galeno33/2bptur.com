<?php
    ...
    require_once('confirmacaoController.php');
    ...
    // Inicie a sessão para acessar a matrícula do usuário
    //session_start();
    $usuario = $_SESSION['matricula'];

    // Instancie a conexão com o banco de dados
    $conn = new Conexao();
    $permutasRepo = new PermutaRepository($conn->getConexao());

    // Obtenha as permutas do usuário
    $permutas = $permutasRepo->getPermutasByUsuario($usuario);
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <meta name="description" content="Minhas Permutas - 2º BPTUR">
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800 font-weight-bold">Minhas Permutas</h1>
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Minhas Permutas Registradas</h6>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Nome do Permutado</th>
                                            <th>Data do Serviço</th>
                                            <th>Autorização</th>
                                            <th>Confirmação da Permuta</th>
                                        </tr>
                                    </thead>
                                    <tfoot>
                                        <tr>
                                    ...
                                    <tbody>
                                        <?php foreach ($permutas as $permuta): ?>
                                            <?php
                                            $id = $permuta->getId();
                                            $matrPermutante = $permuta->getMatrPermutante();
                                            $guerraPermutante = $permuta->getGuerraPermutante();
                                            $guerraPermutado = $permuta->getGuerraPermutado();
                                            $matrPermutado = $permuta->getMatrPermutado();
                                            $dataPermuta = $permuta->getDataPermuta();
                                            $aceitoPermuta = $permuta->getAceitoPermuta();
                                            $autorizacaoPermuta = $permuta->getAutorizacaoPermuta();

                                            // Formatação da data para o padrão brasileiro
                                            $dataFormatada = date('d-m-Y', strtotime($dataPermuta));
                                            ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($guerraPermutante); ?></td>
                                                <td><?php echo htmlspecialchars($guerraPermutado); ?></td>
                                                <td><?php echo htmlspecialchars($dataFormatada); ?></td>
                                                <td style="background: <?php echo $autorizacaoPermuta == 'Em análise' ? '#ffc107' : ($autorizacaoPermuta == 'Autorizado' ? '#007bff' : '#dc3545'); ?>; color: white;">
                                                    <strong><?php echo htmlspecialchars($autorizacaoPermuta); ?></strong>
                                                </td>
                                                <td>
                                                    <?php if ($matrPermutado == $usuario && $aceitoPermuta == "Não confirmado"): ?>
                                                        <div class="d-flex gap-2">
                                                            <a href="../permuta/confirmarPermuta.php?confirmar=<?php echo urlencode($id); ?>" class="btn btn-primary btn-sm">Confirmar</a>
                                                            <a href="../permuta/negarPermuta.php?negar=<?php echo urlencode($id); ?>" class="btn btn-danger btn-sm">Não Confirmar</a>
                                                        </div>
                                                    <?php else: ?>
                                                        <?php echo htmlspecialchars($aceitoPermuta); ?>
                                                    <?php endif; ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
    ...
    <!-- Modal de Confirmação de Permuta (Oculto, pois não está sendo usado no código atual, mas mantido para referência) -->
    <div id="myModal" class="modal fade" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Confirma a autorização da permuta?</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>Clique em <strong>Confirmar</strong> para aceitar a permuta, ou <strong>Não Confirmar</strong> para recusá-la.</p>
                    <div class="modal-footer">
                        <a href="../permuta/confirmarPermuta.php?confirmar=<?php echo urlencode($id); ?>" class="btn btn-primary">Confirmado</a>
                        <a href="../permuta/negarPermuta.php?negar=<?php echo urlencode($id); ?>" class="btn btn-danger" data-dismiss="modal">Não Confirmado</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function () {
            $('#dataTable').DataTable({
                ...
                "pageLength": 5,
                "responsive": true,
                "order": [[2, "desc"]] // Ordenar por "Data do Serviço" em ordem decrescente por padrão
            });
        });
    </script>
</body>
