...
<head>
    ...
    <meta name="description" content="Autorização de Permutas - 2º BPTUR">
    <meta name="author" content="Fabio Galeno">
    <title>Permutas a Autorizar</title>
...
<body id="page-top">
                ...
                <li class="nav-item">
                    ...
                    <div id="collapseAdmin" class="collapse" aria-labelledby="headingAdmin" data-parent="#accordionSidebar">
                        <div class="bg-white py-2 collapse-inner rounded">
                            ...
                            <?php if ($situacao == "HABILITADO") { ?>
                                <a class="collapse-item" href="permutas_autorizadas.php">Permutas Autorizadas</a>
                            <?php } ?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800 font-weight-bold">Autorização de Permutas</h1>
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Permutas a Autorizar</h6>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                                <?php
                                                $autorizarPermuta = new AutorizarPermuta($conn);
                                                $autorizar = $autorizarPermuta->getPermutasEmAnalise();

                                                foreach ($autorizar as $autorizo) {
                                                    $id = $autorizo->getId();
                                                    $matrPermutante = $autorizo->getMatrPermutante();
                                                    $guerraPermutante = $autorizo->getGuerraPermutante();
                                                    $guerraPermutado = $autorizo->getGuerraPermutado();
                                                    $matrPermutado = $autorizo->getMatrPermutado();
                                                    $dataPermuta = $autorizo->getDataPermuta();
                                                    $aceitoPermuta = $autorizo->getAceitoPermuta();
                                                    $autorizacaoPermuta = $autorizo->getAutorizacaoPermuta();

                                                    $dataFormatada = date('d-m-Y', strtotime($dataPermuta));

                                                    if ($aceitoPermuta == "Confirmado") {
                                                        echo "<tr>";
                                                        echo "<td>" . htmlspecialchars($guerraPermutante) . "</td>";
                                                        echo "<td>" . htmlspecialchars($guerraPermutado) . "</td>";
                                                        echo "<td>" . $dataFormatada . "</td>";
                                                        echo "<td>" . htmlspecialchars($aceitoPermuta) . "</td>";
                                                        echo "<td style='display: flex; gap: 10px; justify-content: center;'>";
                                                        echo "<a href='autorizarPermuta.php?autorizado=" . urlencode($id) . "' class='btn btn-primary btn-sm'>Autorizar</a>";
                                                        echo "<a href='negarPermuta.php?negado=" . urlencode($id) . "' class='btn btn-danger btn-sm'>Negar</a>";
                                                        echo "</td>";
                                                        echo "</tr>";
                                                    }
                                                }
    ...
    <!-- Modal de Permuta (Oculto, pois não está sendo usado no código atual) -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Clique em <strong>Autorizar</strong> para aceitar a permuta, ou <strong>Negar</strong> para recusá-la.</p>
                    <div class="modal-footer">
                        <a href="atualizacaoPermutas.php?autorizado=<?php echo urlencode($id); ?>" class="btn btn-primary">Autorizar</a>
                        <a href="atualizacaoPermutas.php?negado=<?php echo urlencode($id); ?>" class="btn btn-danger" data-dismiss="modal">Negar</a>
                    </div>
                </div>
            </div>
        </div>
